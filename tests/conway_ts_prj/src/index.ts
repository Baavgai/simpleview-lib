...
interface Props {
    view: PixelBox;
    bgColor: PixelColor;
    liveColor: PixelColor;
    liveChar: string;
    pattern: string;
}


const Conway = (({ view, bgColor, liveColor, liveChar, pattern }: Props) => {
    view.fill(bgColor);
    pattern.split("\n")
    ...
    view: createPixelBox({ canvasId: "canvas1", width: 64, height: 48, gutterSize: 2, cellSize: 8 }),
    bgColor: "#000",
    liveColor: "#0f0",
    liveChar: "O",
    pattern: `    .
